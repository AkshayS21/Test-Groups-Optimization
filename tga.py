...
def optimize(dataframe):
    ...
    #print("Checking for combinations with all parameter values  <= 0.5... ")
    ...
    if not result_d_df.empty:
        ...
        print(" Combinations with all parameter values  <= 0.5 found!\n Creating dataframe...")
        ...
        try:
            ...
            print("It seems that there was an error while saving the excel file.\n The dataframe has still been generated.")
            print("Please use 'dataframe.to_excel(file_path)' to generate the excel file.")
        ...
        #print("None found. Please enter weights in the dialog box.")
        ...
        opt2.reset_index(drop = 1, inplace = True)
...
def weights(dataframe):
    ...
    Label(frame0, text = "\t(Default value : 1)\t\t").grid(row = 4, column = 1)
...
def gen_excel(dataframe):
    ...
    def save_excel():
        ...
        Label(appc, text = "    ").pack()
        #Label(appc, text = "    ").pack()
        Label(appc, text = "Selected file path:").pack()#grid(row = 2, column = 1)
        Label(appc, text = "    ").pack()
        #Label(appc, text = "    ").pack()
        Label(appc, text = filepath + '.xlsx').pack()#grid(row = 3, column = 1)
    ...
    b1 = Button(appc, text = "Select file path", command = save_excel).pack()#grid(row = 1, column = 1)
...
def create(data, testgroups, splitsize,  decimals = 2):
    ...
    if splitsize * 2 <= testgroups:
        ...
        if testgroups / splitsize == 2:
            ...
            output = func_half(data_new, testgroups, decimals = 2)

        else:

            output = func_other(data_new , testgroups, splitsize, decimals = 2)
...
def func_half(data , testgroups , decimals = 2):
...
def func_other(data , testgroups, splitsize, decimals = 2) :
...
def select_file():
    ...
    def selectxfile():
        ...
        frame2 = LabelFrame(appf, text = "Choose one sheet",padx = 20, pady = 20, height = 400, width = 400)
...
def select_groups(dataframe):
    ...
    def counter():
        ...
        for t in var_list:            
            if t.get() != '':                
                cntlist.append(t.get())
         
        #global count
        
        count = len(cntlist)
        
        Label(frame1 , text = "          ").grid(row = 1, column = 50)
        Label(frame1 , text = str(count)).grid(row = 1, column = 51)
        #return count
    ...
    Label(frame2 , text = "   ").grid(row = 35, column = 6)
    Button(frame2, text="Count selected", command= counter).grid(row=35, column = 7)
    
    Label(frame2 , text = "-------------------").grid(row = 50, column = 5)
    Button(frame2, text="  Add  ", command= add_col).grid(row=60, column = 5)
